...
### 节点选中 — 独立行高亮（Tana-style）

每个被选中或被隐式选中（祖先选中）的节点各自显示独立的行高亮条：

| Token | 值 | 覆盖范围 | 圆角 |
|-------|----|---------|------|
| `--selection-row` | `rgba(139,92,246,0.12)` | 每个被选中/隐式选中的节点行 | `rounded-sm` (4px) |

- 高亮条从 bullet 左沿开始（`depth*28 + 25px`），延伸到行右端
- 直接选中的节点和被祖先覆盖的子节点都独立显示高亮条
- 通过 `ancestorSelected` prop 向下传递选中状态
- 引用节点（reference）使用相同的行高亮
