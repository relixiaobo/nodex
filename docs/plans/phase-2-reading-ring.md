...
> 依赖：Phase 1.5 (node_create / node_read / node_edit / node_search + Chat)
> 可并行：Phase 4 (编排)
...
```
AI 提取当前内容的核心结构/主题
  → node_search（按 tags/fields/date_range 查找候选节点）
  → node_read（渐进式披露——逐级读取候选节点内容）
...
- **node_create children** → `<ref>` 转为 Reference 节点 (`type: 'reference', targetId`)
...
> Phase 1.5 将原 `node` 单工具拆分为 6 个独立工具。详见 `phase-1.5-node-tool-gaps.md`。

| 能力 | Phase 1.5 提供 | Phase 2 使用场景 |
|------|---------------|-----------------|
| `node_create` | 创建节点（支持 children 批量子树 + fields 便利参数） | 创建 #source / #spark / #note 节点 + 一步设 is/has/about |
| `node_read` | 渐进式读取（含 fields 增强 + isReference） | 碰撞时读取候选节点内容 |
| `node_edit` | 修改已有节点（含 fields 便利参数） | 后续补充 is/has/about 字段 |
| `node_search` | 搜索（tags + fields 过滤 + linkedTo 反向引用 + sort） | 碰撞时查找候选节点 |
