...
Nodex 追求的视觉体验是：打开工具如同展开一张温润的纸——没有边框争夺注意力，没有阴影制造层级焦虑，没有色彩叫嚣存在感。结构隐于内容之下，只在需要时浮现。
...
| 序号 | 名称 | 色值 | 约对比度 | 色相描述 |
|------|------|------|----------|----------|
| 0 | Faded Violet | `#7B6B8D` | 4.4:1 | 褪色紫，旧书脊 |
| 1 | Brick Rose | `#9B6E6E` | 3.9:1 | 砖粉，赤陶 |
| 2 | Slate Blue | `#5E7A92` | 4.1:1 | 灰蓝，雨后石板 |
| 3 | Olive | `#697A4D` | 4.2:1 | 橄榄，干叶 |
| 4 | Ochre | `#8A7142` | 4.3:1 | 赭石，土壤 |
| 5 | Deep Indigo | `#515C96` | 5.0:1 | 靛蓝，墨水渍 |
| 6 | Smoke Rose | `#8E5E70` | 4.5:1 | 烟玫瑰，褪色花瓣 |
| 7 | Dark Teal | `#4D7A7A` | 4.0:1 | 暗青，铜绿 |
| 8 | Rust Orange | `#8E6242` | 4.5:1 | 铁锈橙，老铜 |
| 9 | Charcoal | `#616161` | 4.9:1 | 木炭，铅笔芯 |
...
文档流内**一律不使用阴影**。所有层级区分通过边框、背景色差异或空间距离完成。

| 层级 | 场景 | 表现 |
|------|------|------|
| **文档层** | 大纲、面板、内容区 | 无阴影、无边框，纯 Paper 底色 |
| **浮层** | Dropdown、Popover、Tooltip | `border: 1px solid var(--border)` + Surface 底色 |
| **遮罩层** | Modal、Dialog | `border: 1px solid var(--border)` + Surface 底色 + `backdrop: rgba(0,0,0,0.3)` |
...
```
┌─────────────────────────────┐  ← radius-container (8px), p-1 (4px), border
│ ┌─────────────────────────┐ │
│ │  Item text              │ │  ← px-2, py-1
│ └─────────────────────────┘ │
│ ┌─────────────────────────┐ │
...
- 容器：`rounded-lg border border-border bg-surface p-1`（无阴影）
- 项：`px-2 py-1 text-base`
- 选中项：`bg-primary text-primary-foreground`
- 高亮项（键盘导航）：`bg-foreground/4`
...
| 我要加阴影了？ | 文档流内禁止。浮层也不用阴影，用 `border-border` |
