...
四个目录是同一 git repo 的固定 worktree，CLAUDE.md 自动共享。每个 Dev Agent worktree 同一时间 checkout 一个 feature branch。

- **角色由分支决定**：`main` = nodex（不直接写大功能代码）；feature branch = Dev Agent（执行 PR body 中的任务）
...
```
用户提需求 → ① nodex 规划 → ② nodex 备料 → ③ 用户在 Dev Agent 客户端执行 → ④ nodex Review & Merge
```

**① 规划**（nodex）：分析需求，涉及 3+ 文件先出计划，用户确认方向。
...
**③ 执行**（用户 + Dev Agent）：
- 用户在对应工作区 checkout feature branch，启动 Dev Agent
- Dev Agent 读 PR description 获取完整上下文
- 自行 `typecheck` → `vitest` → `build` 自验
- 完成后 push + `gh pr ready`
- **禁止 Dev Agent 合并 PR**
...
nodex 可直接在 main 上修复，或使用 Claude Code subagent 完成，无需走 PR 流程。
