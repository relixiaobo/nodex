...
- **暗色优先**：深色背景长时间阅读不疲劳，亮色作为备选
- **克制用色**：仅 2 个强调色（主色 + 辅助色），大面积留给灰阶
- **电光感**：强调色饱和度高、对比度高，在深色背景上"发光"
...
主色选择 **靛紫（Indigo-Violet）** —— 延续现有品牌认知，略微偏紫提升辨识度。
...
| `--primary` | `#7C6BF4` | 主色（靛紫，介于 indigo 和 violet 之间） |
| `--primary-hover` | `#8D7FF7` | 主色 hover |
| `--primary-muted` | `rgba(124,107,244,0.15)` | 主色背景（选中行、badge） |
| `--primary-foreground` | `#FFFFFF` | 主色上的文本 |
| `--accent` | `#2DD4A8` | 辅助色（青绿） |
...
| `--primary` | `#6C5CE7` | 主色（稍深，保证白底对比度） |
| `--primary-hover` | `#5A4BD6` | 主色 hover |
| `--primary-muted` | `rgba(108,92,231,0.08)` | 主色背景 |
| `--primary-foreground` | `#FFFFFF` | 主色上的文本 |
...
|------|------|-----------|---------|------------|----------|
| 0 | Indigo | `#A5A0FC` | `rgba(124,107,244,0.15)` | `#6C5CE7` | `rgba(108,92,231,0.08)` |
| 1 | Pink | `#F472B6` | `rgba(236,72,153,0.15)` | `#DB2777` | `rgba(219,39,119,0.08)` |
| 2 | Violet | `#C084FC` | `rgba(139,92,246,0.15)` | `#7C3AED` | `rgba(124,58,237,0.08)` |
...
| FieldRow 左偏移 | 21px | 6 (基准) + 15 (bullet 宽度) |
...
| Bullet | 21 × 15px | 与行高对齐 |
| Chevron | 21 × 15px | 与行高对齐 |
...
| 选中 (selected, reference only) | `ring-1 ring-primary/40 rounded-sm bg-primary/5` 包裹 bullet + 文本 |
| 拖拽中 | `opacity-50`，目标行显示蓝色插入线 |
| Done (完成) | `line-through opacity-50` |
...
在 `main.css` 的 `@theme` 块中定义 light mode 为默认，通过 `[data-theme="dark"]` 或 `@media (prefers-color-scheme: dark)` 覆盖：
