...
export const editTool: AgentTool<typeof editToolParameters, unknown> = {
  ...
  label: 'Edit Node',
  description: [
    'Modify an existing node. Only provided parameters are applied. Semantics are incremental (add/set), not replace.',
    '',
    'text uses the same format as node_create, with these edit-specific rules:',
    '- Line 1 with plain text = rename the node.',
    '- Line 1 that is ONLY #tag, [X]/[ ], or field:: = does NOT rename, only applies that change.',
    '- #tag = add tag to the node.',
    '- field:: value = set field value.',
    '- field:: (empty, no indented values below) = clear that field.',
    '- [X] = check, [ ] = uncheck.',
    '- Indented lines = append new children (does NOT remove or replace existing children).',
    '',
    'To add children WITHOUT renaming, use a tag or checkbox as the first line:',
    '  text: "#existingtag\\n  New child 1\\n  New child 2"',
    'Or set fields without renaming:',
    '  text: "Status:: In Progress"',
    '',
    'Examples:',
    '- Rename: text: "New name"',
    '- Rename + add tag: text: "New name #important"',
    '- Add tag + set field + check: text: "#task\\nStatus:: Done\\n[X]"',
    '- Set field only: text: "Priority:: High"',
    '- Clear field: text: "Priority::"',
    '- Add children under existing node: text: "#task\\n  Subtask 1\\n  Subtask 2"',
    '',
    'Other parameters:',
    '- removeTags: tag names to remove (text cannot express deletion).',
    '- parentId + afterId: move the node to a new location.',
    '- data: set non-content properties (description, color, codeLanguage, showCheckbox).',
    '',
    'Search nodes: rule editing not supported. Delete and recreate with node_create(type: "search").',
    '',
