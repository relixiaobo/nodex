...
export const createTool: AgentTool<typeof createToolParameters, unknown> = {
  ...
  description: [
    ...
    '',
    'Text rules:',
    '- First line = root node name. May include inline #tags, [[Name^nodeId]] references, or leading [X] / [ ].',
    '- If the first line is exactly [[Name^nodeId]], a reference node is created instead.',
    '- Subsequent top-level lines: only #tags, [X]/[ ], or field:: lines (not new nodes).',
    '- Child nodes: indent 2 spaces per level. Max depth = 3.',
    '- field:: value = single-value field.',
    '- field:: (no value) + indented lines = multi-value field.',
    '- #tagName = apply tag (auto-creates if not found).',
    '- [[Display^nodeId]] = reference to existing node (resolved by nodeId).',
    '- [X] = checked, [ ] = unchecked.',
    '- Lines starting with "- " are accepted; the prefix is stripped.',
    '- data parameter: set non-content properties (description, color, codeLanguage, showCheckbox).',
    '',
    'Examples:',
    '- Simple: text: "Buy groceries #task"',
    '- With field: text: "Meeting #meeting\\nStatus:: Done"',
    ...
    '- rules.searchTags = tag filter (AND logic; unresolved tags are skipped)',
    '- rules.fields = field value filter (unresolved fields are skipped)',
    ...
    '- rules.sortBy = created|modified|name|refCount[:asc|desc]',
    '',
