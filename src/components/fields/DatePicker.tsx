  ...
  <div>
   ...
   <div className="flex items-center justify-between mb-2">
    ...
    <div className="flex items-center gap-1">
     {onToday && (
      <button
       className="rounded-md px-1.5 h-7 text-xs text-foreground-secondary hover:bg-foreground/4 transition-colors cursor-pointer"
       onClick={onToday}
      >
       {t('datePicker.today')}
      </button>
   ...
   <div className="grid grid-cols-7 gap-1">
    ...
    {DAY_HEADERS.map((d, i) => (
     <div
      key={`h${i}`}
      className={`aspect-square flex items-center justify-center text-xs font-medium ${i === 0 || i === 6 ? 'text-destructive/40' : 'text-foreground-tertiary'
       }`}
     >
      {d}
     </div>
