...
| 层 | Token | 值 | 覆盖范围 | 圆角 |
|----|-------|----|---------|------|
| **子树遮罩** | `--selection` | `rgba(139,92,246,0.10)` | 选中节点行 + 所有展开的子节点 | `rounded-sm` (4px) |
| **行高亮** | `--selection-row` | `rgba(139,92,246,0.18)` | 仅选中的根节点行 | `rounded-sm` (4px) |

- 两层从 bullet 左沿开始（`depth*28 + 25px`），延伸到行右端
- 子树遮罩 z-index 在内容下方（`z-0`），行高亮在内容层（`z-[1]` 内）
- 多选时每个根选中节点各自独立双层高亮
- 引用节点（reference）使用相同的双层高亮（不再使用紧凑 ring）
