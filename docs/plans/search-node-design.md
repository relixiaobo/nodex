...
- **存放位置**：创建后自动放入 `SEARCHES` 系统容器（固定容器 ID `CONTAINER_IDS.SEARCHES`，已在 `system-node-registry.ts` 注册）
...
| `SEARCHES` 系统容器 | `src/types/node.ts` → `CONTAINER_IDS.SEARCHES` | 固定容器 ID，已定义 |
...
- ~~候选集排除~~ → 3.4 节
- ~~根条件约定~~ → 2.2 节
- ~~容器命名~~ → 1.2 节（固定 `CONTAINER_IDS.SEARCHES`）
