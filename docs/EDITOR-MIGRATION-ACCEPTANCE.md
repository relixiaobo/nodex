...
- [ ] 鼠标点击文本中某位置 → 光标精确定位到点击处[备注：点击node 末尾的空白位置，光标会出现在末尾，再次点击末尾空白位置，光标又会移动到开头，不符合预期]
- [x] ArrowUp 到达节点 → 光标在末尾
- [ ] ArrowDown 到达节点 → 光标在开头[备注：不知道如何操作测试，不详细]
- [x] 新建空节点 → 光标在开头
- [x] 选择模式按字符键进入编辑 → 字符被插入（pendingInputChar 消费正确）
- [x] 引用节点单击仅选中（不进编辑）；双击才进入编辑模式
...
| **ArrowUp（行首）** | 光标在行首按 ↑ → 焦点到上一可见节点 | [x] |
| **ArrowDown（行尾）** | 光标在行尾按 ↓ → 焦点到下一可见节点 | [x] |
| **ArrowUp（非行首）** | PM 默认行为（单行无效果） | [备注：预期是移动到上一节点的末尾] |
...
| 触发器 | 验收步骤 | 通过 |
|--------|---------|------|
| **# HashTag** | 输入 `#ta` → 标签选择器出现，显示匹配标签 | [备注：出现了标签选择器，但有三问题，第一个是选择器背后的node中的文本穿模到了选择器中，另一个是选择器中的tag无法通过选择，只能通过 enter 选择，第三个问题如果在一个空白节点（bullet为灰色）中输入 #，光标会移动到 # 的前面] |
...
| **@ 选择（空节点）** | 空节点输入 `@ref` 选择 → 节点转为引用节点 | [x，出现了，只能通过enter选择，不能通过点击选择，另外如果在一个空白节点（bullet为灰色）中输入 @，光标会移动到 @ 的前面] |
| **@ 选择（有文本）** | 文本中输入 `@ref` 选择 → 插入 inline ref atom | [x] |
| **@ 强制创建** | Cmd+Enter → 创建新节点引用 | [x] |
| **/ Slash** | 输入 `/` → Slash 命令菜单出现 | [x] |
| **/ 触发条件** | 仅空白节点可触发 slash 菜单；已有文本的节点不触发 | [x] |
...
- [x] 粘贴纯文本 → 正确插入
- [ ] 粘贴多行文本 → 换行替换为空格（单行节点约束）[备注： 预期是转换为多个节点，如果是 markdown 格式或者 有层级的无序列表，也需要根据层级进行转换，我觉得可以新建一个任务，后续再实现，这里先保持当前的逻辑]
- [ ] 粘贴富文本 → 只保留纯文本（strip HTML）[备注： 同"粘贴多行文本"的预期，可以一起写入任务]
...
- [x] 编辑器失焦 → 工具栏消失
- [ ] 选区跨越 inline ref atom → 工具栏仍然出现（TextSelection 范围检测）[备注：没看懂这一点我需要测试什么]

### 3.2 FloatingToolbar 定位

- [ ] 工具栏水平居中于选区[备注：跨行选区场景下仍有偏差。当前从本行最后一个字符拖到下一行开头字符时，工具栏会靠近整段中部；预期应跟随鼠标最后落点字符（拖选）或双击选中的落点字符]
- [x] 工具栏不超出视口左边界
- [x] 工具栏不超出视口右边界
- [x] 工具栏在选区上方 40px
...
### 3.4 FloatingToolbar 链接编辑（不纳入本次验收）

- [-] 本节需求已取消（产品决策：移除链接格式编辑能力），本次迁移不验收 Link/Unlink/URL 输入相关行为。

### 3.5 TrailingInput（白话测试步骤）

- [ ] 展开一个父节点，把鼠标移到它子节点列表最底部空白处，确认出现一个输入框（占位文案类似 "Add a node..."）。
- [ ] 在这个输入框里输入 `abc` 后按 Enter，确认会在该父节点下新增一个子节点，内容是 `abc`。
- [ ] 输入框留空直接按 Enter，确认也会新增一个空子节点（用于连续录入）。
- [ ] 在输入框按 Tab，确认输入框整体向右缩进一层（只是位置变化，不会立刻创建节点）。
- [ ] 再按 Shift+Tab，确认输入框向左回退一层。
- [ ] 输入一些文字后点击其他区域（触发 blur），确认文字不会丢失，会被创建为新子节点。
- [ ] 在空输入框按 Escape，确认输入框关闭/失焦，不会额外创建节点。
- [ ] 如果当前父节点是 Options 字段场景：输入时应能出现候选项，方向键+Enter 可选择候选。
