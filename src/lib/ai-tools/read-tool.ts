...
export const readTool: AgentTool<typeof readToolParameters, unknown> = {
  ...
  description: [
    ...
    '',
    'Schema operations (modifying tag/field definitions):',
    '1. node_read(nodeId: "schema") → find tagDef by name',
    '2. node_read(nodeId: tagDefId) → find fieldDef children',
    '3. node_edit(nodeId: fieldDefId, data: {fieldType: "options"}) → change field type',
    '4. node_create(text: "Option name", parentId: fieldDefId) → add field option',
  ].join('\n'),
