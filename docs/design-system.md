...
| 层 | Token | 值 | 覆盖范围 | 圆角 |
|----|-------|----|---------|------|
| **子树遮罩** | `--selection` | `rgba(139,92,246,0.10)` | 选中节点展开时覆盖整个子树区域（含边框 `primary/12%`），子节点形成一个连续整体 | `rounded-sm` (4px) |
| **直接行高亮** | `--selection-row` | `rgba(139,92,246,0.18)` | 仅直接选中的行 | `rounded-sm` (4px) |
