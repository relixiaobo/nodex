...
export function OutlinerItem({ nodeId, depth, rootChildIds, parentId, rootNodeId, fieldDataType, attrDefId, onNavigateOut, bulletColor }: OutlinerItemProps) {
  ...
  return (
    <div role="treeitem" aria-expanded={isExpanded} className="relative">
      {/* Selection subtree mask: covers parent row + children as one bordered unit */}
      {isSelected && !isFocused && isExpanded && (
        <div
          className="absolute top-0 bottom-0 right-0 bg-selection rounded-sm border border-primary/[0.15] pointer-events-none z-0"
      ...
      >
        ...
        {showRowHighlight && (
          <div
            className={`absolute right-0 bg-selection-row rounded-sm pointer-events-none ${!isExpanded ? 'border border-primary/[0.15]' : ''}`}
            style={{ left: depth * 28 + 6 + 15, top: isExpanded ? 0 : 1, bottom: isExpanded ? 0 : 1 }}
          />
        )}
