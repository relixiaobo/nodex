...
SearchNode (doc_type: 'search')
  ...
  ├── meta: [searchExprTupleId, viewTupleId, ...]  ← 搜索配置
...
> **搜索条件 = Tuple 节点树，不是 DSL 字符串。**

搜索表达式通过 Tuple 嵌套结构表达，**不要**把搜索条件序列化为 DSL 字符串（如 `"#task AND status:TODO"`）。

**Tuple 树结构**（AND/OR/NOT 组合）：

```
SearchNode meta:
  └── Tuple [SYS_A15, tagDefId, logicGroupTupleId]
        └── logicGroupTuple (AND 组)
              ├── children[0]: AND 操作符标识
              ├── children[1]: conditionTuple₁ [fieldDefId, operator, value]
              ├── children[2]: conditionTuple₂ [fieldDefId, operator, value]
              └── children[3]: nestedOrTuple (OR 子组，递归嵌套)
```
...
**实现指引**：Query Builder 的每个条件行对应一个 Tuple 节点，逻辑组合（AND/OR/NOT 按钮）= 创建/修改父 Tuple 的嵌套关系。条件的增删改 = 标准的树操作（addChild / removeChild / updateNode）。
...
| 函数 | 功能 |
|------|------|
| `getSearchConfig(searchNodeId)` | 从 meta 提取搜索配置 |
