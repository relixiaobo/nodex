...
export const createTool: AgentTool<typeof createToolParameters, unknown> = {
  ...
  description: [
    ...
    '',
    'text is a multi-line plain text string. Each line is one node or field entry.',
    'Hierarchy is expressed by 2-space indentation. This is NOT markdown.',
    '',
    'Format:',
    '  Line 1 = root node name (required)',
    '  Indented lines = children, fields, or field values of their parent',
    '  Each indent level = 2 spaces deeper than parent',
    '',
    'Syntax within a line:',
    '  #tagName          → tag (auto-created if not found)',
    '  field:: value     → set field to value',
    '  field::           → multi-value field (values on next-indented lines) or clear field',
    '  [[Name^nodeId]]   → reference (entire line = ref node; inline = inline ref)',
    '  [X] or [ ]        → checkbox (checked / unchecked)',
    '',
    'Complete example:',
    '```',
    ...
    '    [ ] Review budget #task',
    '  Related: see [[Q1 Plan^id456]]',
    '```',
    'This creates:',
    '  root "Weekly review" tagged #meeting',
    '    field "Date" = "2026-03-21"',
    '    field "Attendees" = ["Alice", "Bob", ref to Charlie]',
    '    child "Action items"',
    '      child "Finish report" tagged #task, checked',
    '      child "Review budget" tagged #task, unchecked',
    '    child "Related: see Q1 Plan" with inline ref',
    '',
    'Key rules:',
    '- After a multi-value field:: line (no inline value), ALL subsequent lines at the next indent level are field values — not children. They become children again only when indentation returns to the field level or lower.',
    '- Lines at indent 0 after line 1 can only be root metadata: #tags, [X]/[ ], or field:: lines.',
    '- Max depth: 3 levels of nesting.',
    '- Lines starting with "- " are accepted (prefix is stripped).',
    ...
    '',
    'rules fields (all optional, at least one required):',
    '  query        string      Text filter on node name/description',
    '  searchTags   string[]    Tag display names, AND logic. Unknown tags are skipped.',
