...
- [ ] 纯文本节点：编辑器显示正确文本
- [ ] 带 bold 格式的节点：编辑器显示粗体
- [ ] 含内联引用的节点：引用显示为蓝色 chip，不可编辑
- [ ] 空节点：编辑器为空，光标可输入

### 2.3 光标定位

- [ ] 鼠标点击文本中某位置 → 光标精确定位到点击处
- [ ] ArrowUp 到达节点 → 光标在末尾
...
| 快捷键 | 验收步骤 | 通过 |
|--------|---------|------|
| **Enter（行尾）** | 光标在行尾按 Enter → 下方出现空节点，光标在新节点 | [ ] |
| **Enter（行中）** | 光标在 "Hel\|lo" 中间按 Enter → 当前节点变为 "Hel"，新节点为 "lo" | [ ] |
| **Enter（行中带格式）** | 光标在 `<strong>He\|llo</strong>` 中按 Enter → 两个节点都保持 bold | [ ] |
...
| **ArrowDown（行尾）** | 光标在行尾按 ↓ → 焦点到下一可见节点 | [ ] |
| **ArrowUp（非行首）** | PM 默认行为（单行无效果） | [ ] |
| **Escape** | 按 Esc → 退出编辑，进入选择模式，当前节点高亮 | [ ] |
| **Shift+ArrowUp** | 按 Shift+↑ → 保存内容，进入选择模式，范围扩展 | [ ] |
| **Shift+ArrowDown** | 同上，向下扩展 | [ ] |
| **Cmd+A（第一次）** | 按 Cmd+A → 选中编辑器内全部文本 | [ ] |
| **Cmd+A（第二次）** | 全选状态再按 Cmd+A → 退出编辑，选中所有兄弟节点 | [ ] |
| **Cmd+Shift+ArrowUp** | 当前节点上移 | [ ] |
| **Cmd+Shift+ArrowDown** | 当前节点下移 | [ ] |
| **Cmd+Enter（无下拉）** | toggle checkbox 状态 | [ ] |
| **Ctrl+I** | 进入描述编辑模式（Mac 上 Ctrl 键，非 Cmd） | [ ] |

### 2.6 格式快捷键

| 快捷键 | 验收步骤 | 通过 |
|--------|---------|------|
| **Cmd+B** | 选中文本 → Cmd+B → 变为粗体 | [ ] |
| **Cmd+I** | 选中文本 → Cmd+I → 变为斜体（Mac 上 Cmd+I = Mod+I） | [ ] |
| **Cmd+E** | 选中文本 → Cmd+E → 变为行内代码 | [ ] |
| **Cmd+Shift+S** | 选中文本 → 变为删除线 | [ ] |
| **Cmd+Shift+H** | 选中文本 → 变为高亮 | [ ] |

### 2.7 撤销/重做

- [ ] Cmd+Z → 撤销最近的文字输入
- [ ] Cmd+Shift+Z / Cmd+Y → 重做
- [ ] 连续输入后 Cmd+Z → 合理的撤销粒度（不是逐字符）
- [ ] 格式变更可撤销
- [ ] 撤销后光标位置正确
...
| 触发器 | 验收步骤 | 通过 |
|--------|---------|------|
| **# HashTag** | 输入 `#ta` → 标签选择器出现，显示匹配标签 | [ ] |
| **# 选择** | 选择标签 → `#ta` 文本被清除，标签应用到节点 | [ ] |
| **# 强制创建** | `#newTag` → Cmd+Enter → 创建新标签并应用 | [ ] |
| **# 取消** | `#ta` → Escape → 选择器关闭，文本保留 | [ ] |
| **# 下拉导航** | ↑↓ 在选择器中移动高亮 | [ ] |
| **# 守卫** | 聚焦已含 `#` 的节点 → 不自动触发选择器 | [ ] |
| **@ Reference** | 输入 `@no` → 引用选择器出现 | [ ] |
| **@ 选择（空节点）** | 空节点输入 `@ref` 选择 → 节点转为引用节点 | [ ] |
| **@ 选择（有文本）** | 文本中输入 `@ref` 选择 → 插入 inline ref atom | [ ] |
| **@ 强制创建** | Cmd+Enter → 创建新节点引用 | [ ] |
| **/ Slash** | 输入 `/` → Slash 命令菜单出现 | [ ] |
| **/ URL 不误触** | 输入 `https://` → 不触发 slash 菜单（URL 中的 / 不匹配） | [ ] |
| **> Field** | 空节点输入 `>` → 立即创建字段（fire-once） | [ ] |
| **> 不重复** | 输入 `>` 触发后，继续输入 `>a` 不再次触发 | [ ] |

### 2.9 粘贴

- [ ] 粘贴纯文本 → 正确插入
- [ ] 粘贴多行文本 → 换行替换为空格（单行节点约束）
- [ ] 粘贴富文本 → 只保留纯文本（strip HTML）
...
- [ ] 选中文本 → 工具栏出现在选区上方
- [ ] 取消选中 → 工具栏消失
- [ ] 拖拽选择过程中 → 工具栏不显示（mousedown 隐藏，mouseup 后显示）
- [ ] 编辑器失焦 → 工具栏消失
- [ ] 选区跨越 inline ref atom → 工具栏仍然出现（TextSelection 范围检测）
...
| 按钮 | 验收步骤 | 通过 |
|------|---------|------|
| **Bold** | 选中文本 → 点击 Bold → 文本变粗体；再点击 → 取消粗体 | [ ] |
| **Italic** | 同上，斜体 | [ ] |
| **Strikethrough** | 同上，删除线 | [ ] |
| **Code** | 同上，行内代码 | [ ] |
| **Highlight** | 同上，高亮 | [ ] |
| **Heading** | 同上，heading mark | [ ] |
| **所有按钮状态** | 选中已格式化文本 → 对应按钮高亮 | [ ] |
| **点击不失焦** | 点击工具栏按钮 → 编辑器不失焦，选区保留 | [ ] |

### 3.4 FloatingToolbar 链接编辑
...
- [ ] Blur 时有内容 → 创建子节点（保护用户输入）
- [ ] Placeholder 文本正确显示（"Add a node..."或类似）
- [ ] Options 字段的自动补全功能正常（如果适用）
