...
**不变的**：Chat session 管理（ai-persistence）、Agent 核心（pi-agent-core）、ChatInput / ChatMessage 组件、Debug 模式、消息树/分支/压缩。

**小改的**：`ai-chat-tree.ts` 的 `createSession()` 需要接受可选 `id` 参数（见 §3）；`ai-service.ts` 的 `configureAgent()` 需要导出（见 §3）。
...
```typescript
// ai-service.ts — 新增（需要先 export configureAgent）
...
export async function restoreChatSessionById(
  ...
  if (!runtime.restorePromise) {
    runtime.restorePromise = (async () => {
      await configureAgent(agent);           // ← 需要先 export（当前未导出）
      ...
      if (session) {
        ...
        const newSession = createSession(sessionId);  // ← 需要给 createSession 加可选 id 参数
...
// ── 前置改动 ──
// 1. ai-service.ts: 给 configureAgent() 加 export 关键字（当前是内部函数）
// 2. ai-chat-tree.ts: createSession(id?: string) 加可选 id 参数：
...
| `use-chat-shortcut.ts` | `toggleChat()`（同步） | `void focusOrOpenChat()`（async，用 void 忽略 Promise） |
...
**小改的**：`ai-chat-tree.ts`（`createSession` 加可选 `id` 参数）。

**不改的**：`ai-persistence.ts`、`ChatInput.tsx`、`ChatMessage.tsx`、`ToolCallBlock.tsx`、`ToolbarUserMenu.tsx`（PR #143 已清理）。
...
- [ ] `ai-chat-tree.ts` — `createSession(id?: string)` 加可选 `id` 参数
- [ ] `ai-service.ts` — 导出 `configureAgent` + `agentRegistry` + `getAgentForSession()` + `restoreChatSessionById()`
- [ ] `use-agent.ts` — 新增可选 `sessionId` 参数
- [ ] `ui-store.ts` — 删除 `chatOpen` 系列，`hasBackingNode` 加 `chat:`，删除 NavigationEvent 注释中 "Chat events are NOT included"
