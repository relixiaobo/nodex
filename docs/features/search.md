...
> **Phase 1 已实现** (2026-02-24)
>
> 单标签搜索完整可用：`?` 触发 → 选择标签 → 创建搜索节点 → 动态显示匹配结果。
> 搜索引擎基于 Loro 内存遍历，支持 AND/OR/NOT 逻辑 + 多态标签搜索。
...
| 层次 | 状态 | 说明 |
|------|------|------|
| 类型定义 | ✅ | `NodeType` 包含 `'search'` + `'queryCondition'`（`src/types/node.ts`） |
...
| `useNodeSearch` hook | ✅ | 内存子串过滤 + 面包屑（跳过 `queryCondition` 和 `search` 类型） |
| Slash command 注册 | ✅ | `search_node` 已启用（`enabled: true`，`shortcutHint: '?'`） |
| 搜索查询引擎 | ✅ | `src/lib/search-engine.ts`：递归条件树评估 + 多态标签搜索 |
| 查询条件节点类型 | ✅ | `queryCondition` 类型 + `queryLogic`/`queryOp`/`queryTargetTag` 属性 |
| `?` 触发 | ✅ | RichTextEditor 触发检测 → TagSelector → `createSearchNode()` |
| 搜索节点创建 | ✅ | `node-store.ts` `createSearchNode(parentId, afterId, tagDefId)` |
| 搜索结果渲染 | ✅ | `SearchResultsChildren` 组件 + `useSearchResults` hook |
| 搜索节点 bullet | ✅ | BulletChevron `isSearch` 放大镜图标 |
| 搜索结果 bullet | ✅ | 引用样式（虚线圆环 ⊙）通过 `isSearchResult` prop |
| Vitest 测试 | ✅ | `tests/vitest/search-engine.test.ts`（13 tests） |
| Seed 数据 | ✅ | SEARCHES 容器包含 `#Task` 搜索节点 |
...
| 2026-02-24 | Phase 1 完成 | `?` 触发 → TagSelector → createSearchNode → 放大镜 bullet → 动态结果（引用样式） |
| 2026-02-24 | 搜索引擎支持 TODO/DONE 操作符（虽 Phase 1 UI 未暴露） | 引擎设计面向 Phase 2，减少后续改动 |
| 2026-02-24 | 无 Query Builder 缓存（trash 排除每次重算） | Phase 1 节点规模小（<1000），premature optimization avoided |
