...
| nodex-cc | _(idle)_ | — | — |
| nodex-cc-2 | _(idle)_ | — | — |
| nodex-codex | 系统节点统一模型与锁定约束（home/container 作为普通 node） | `codex/system-node-capabilities` | `src/types/node.ts`, `src/stores/node-store.ts`, `src/components/panel/NodePanel.tsx`, `src/lib/tree-utils.ts`, `tests/vitest/*`, `docs/TASKS.md` |
...
> **Owner**: nodex-codex | **Branch**: `codex/system-node-capabilities` | **Files**: `src/types/node.ts`, `src/stores/node-store.ts`, `src/components/panel/NodePanel.tsx`, `src/lib/tree-utils.ts`, `tests/vitest/*`, `docs/TASKS.md`

- [ ] 定义集中化系统节点能力判断（container / workspace home）
- [ ] store 层 hard guard：阻止系统节点删除/重命名/移动
- [ ] UI 层 soft guard：隐藏或禁用删除/重命名入口
- [ ] 补 Vitest 回归（系统节点受限、普通节点不受限）
