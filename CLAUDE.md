...
- Bug 修复：关闭对应 GitHub Issue（`gh issue close <N>`），有 PR 的用 `fixes #N` 自动关闭
...
详细协作规范见 `docs/AGENT-COLLABORATION.md`（文档治理、issue 格式、交接协议、冲突预防）。

### 快速参考

- **角色分工**: `nodex`（主 clone）= Review + Merge；`nodex-codex` / `nodex-cc` / `nodex-cc-2` = 功能开发 + 提 PR
- **Git 分支**: `codex/<feature>`、`cc/<feature>`、`cc2/<feature>`，功能开发通过 PR 合入 main
- **Issue 跟踪**: [GitHub Issues](https://github.com/relixiaobo/nodex/issues) + [Nodex Board](https://github.com/users/relixiaobo/projects/1)
- **任务认领**: Dev Agent 自助认领 GitHub Issue（无 `agent:*` label + 最高优先级），用户可随时干预
- **Draft PR**: 开发开始后立即创建 Draft PR，让进度和修改文件对所有人可见
- **Review 驱动**: Dev Agent 完成后加 `needs-review` label → nodex 自行检查并 review → 意见直接写 PR comment
- **文档所有权**: 改功能同步更新 `docs/features/*.md`，改测试同步更新 `docs/TESTING.md`
- **高风险文件**: `node-store.ts`、`OutlinerItem.tsx`、`system-nodes.ts` — 同一时间只有一个 Agent 改，开发前在 Issue 声明文件锁
- **Dev Server 端口**: 主 clone `5199`，nodex-codex `5200`，nodex-cc `5201`，nodex-cc-2 `5202`
- **Session 启动**: 识别身份 → git pull → 检查 open PR comment → 检查待办（详见 `docs/AGENT-COLLABORATION.md`）
...
- Commit message 关联 GitHub Issue：修复 bug 用 `fixes #N`（自动关闭），功能进展用 `ref #N`（仅关联）
...
- **Bug/Feature 跟踪**: [GitHub Issues](https://github.com/relixiaobo/nodex/issues) + [Nodex Board](https://github.com/users/relixiaobo/projects/1)
...
- 每次修复 bug，补一个对应 Vitest 回归用例（与 GitHub Issue 编号对应）
