...
### 节点选中 — 子树遮罩 + 独立行高亮（Tana-style）

选中节点使用两层半透明叠加，均基于 primary 色：

| 层 | Token | 值 | 覆盖范围 | 圆角 |
|----|-------|----|---------|------|
| **子树遮罩** | `--selection` | `rgba(139,92,246,0.08)` | 选中节点展开时覆盖整个子树区域，无边框，子节点形成一个连续整体 | `rounded-b-sm` (底部 4px) |
| **直接行高亮** | `--selection-row` | `#E8E0FA`（不透明） | 仅直接选中的行，带 `border-primary/15%` 边框（荧光感） | `rounded-sm` (4px) |

- 行高亮有 1px 垂直内缩（`top: 1; bottom: 1`），相邻行产生 2px 可见间隙
- 子树遮罩有 1px 底部内缩（`bottom: 1`），与下一兄弟节点保持 2px 一致间隙
- 子树遮罩仅在节点展开时显示（`isExpanded`），子节点作为连续整体呈现，无间隙
- 子节点**不单独渲染行高亮**，仅由子树遮罩覆盖，颜色更浅
- 引用节点（reference）使用相同方案
- reference 单击选中框（`node-selected-ref`）必须使用绝对定位 overlay（`::before`）绘制，不可给布局盒添加 `padding/margin/border`，避免选中瞬间行高变化
...
- **无边框**：避免在已有边框的容器内产生"线框套线框"效果
- 背景：`bg-foreground/[0.06]`（微灰色底）
- 圆角：`radius-md` (6px)
- 高度：20px (`h-5`)，最小宽度 20px (`min-w-5`)
- 内边距：`px-1.5`
- 字号：`text-[10px]`，`font-medium`
- 文本色：`--foreground-tertiary`
- 可点击变体（如 Esc 关闭按钮）：hover 时 `bg-foreground/10 text-foreground-secondary`
