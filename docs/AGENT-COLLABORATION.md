...
# 同时在 issue 中留 comment 说明计划和预计修改的文件
gh issue comment <N> --body "计划：... 预计修改文件：..."
...
gh issue comment <N> --body "卡住了，原因：... 已尝试：... 建议下一步：..."
```
...
Agent 开始开发时，在 GitHub Issue comment 中声明将修改的热点文件：
...
| 环节 | 机制 |
|------|------|
| 任务分配 | Agent 自助认领 GitHub Issue（用户可随时干预） |
| 进度了解 | 用户看 Draft PR / Issue comments / Nodex Board |
| 触发 review | Dev agent 加 `needs-review` label，nodex 自行检查 |
| 传达 review 意见 | nodex 直接在 PR comment 中写，dev agent 下次 session 自行读取 |
| 冲突协调 | Agent 自行声明文件锁，有重叠时用户介入 |

**用户保留的核心职责**：
- 调整优先级（修改 Issue labels / milestone）
- 方向纠偏（在 Issue 或 PR 中 comment）
- 最终合并/拒绝决策（可委托 nodex 执行）
- 指定特定 Agent 做特定任务（覆盖自助认领）
