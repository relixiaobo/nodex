...
- **不加用户没要求的东西**：不要主动加额外 feature、冗余配置项或复杂检测逻辑。但同一行为模式出现在两个以上组件时，必须抽象（见下方「架构决策：补丁 vs 重构」）
- **匹配已有模式**：改 UI 或加功能前，先看 codebase 里同类组件怎么做的（样式、交互、数据流），复用现有模式。不要发明新风格再让用户对齐
- **跨层一致性**：改颜色、名称、类型等共享值时，先 grep 全部引用点（代码、CSS、种子数据、content script、文档），一次改完。不要只改一处就交差
...
| Agent | 客户端 | 工作区 | 角色 |
|-------|--------|--------|------|
| nodex | Claude Code | `nodex/` (main) | 规划 + 创建 PR + Review + Merge + 视觉验证 |
| Claude dev | Claude Code | `nodex-claude/` (feature branch) | 编码执行 |
| Codex dev | Codex 客户端 | `nodex-codex/` (feature branch) | 编码执行 |
| Gemini dev | Antigravity IDE | `nodex-gemini/` (feature branch) | UI 实现 |

四个目录是同一 git repo 的固定 worktree，CLAUDE.md 自动共享。

**角色判定**：根据你的工作目录名自动确定角色——
- `nodex/` → 你是 **nodex**（规划 + 创建 PR + Review + Merge）
- `nodex-claude/` / `nodex-codex/` / `nodex-gemini/` → 你是 **Dev Agent**（编码执行）
...
> **如果你在 `nodex-claude/`、`nodex-codex/` 或 `nodex-gemini/` 目录中，以下流程适用于你。**

1. 运行 `gh pr view --json body -q .body` 读取当前分支的 PR description — 这是你的**完整任务指令**
2. 按 PR body 中的 Checklist 逐步实现
3. 每完成一步 `npm run typecheck`，全部完成后 `npm run verify`（typecheck → test-sync → test → build）
4. 主动 commit，push 到远端
5. `gh pr ready` 标记完成

**禁止事项**：
- **不要 merge PR**（`gh pr merge` 由 nodex 执行）
- **不要修改 `docs/TASKS.md`**（由 nodex 维护）
- **不要 push 到 main**

### nodex 工作流程

> **如果你在 `nodex/` 目录中，以下流程适用于你。**

**备料（为 Dev Agent 准备任务）**：
- 分析需求，涉及 3+ 文件先出计划，用户确认方向
- 创建 feature branch + Draft PR（PR body 是 Dev Agent 的**唯一指令来源**，必须自包含）
- PR body 包含：Summary、Exact Behavior（GIVEN/WHEN/THEN）、文件清单、Checklist、Test plan、注意事项
- 更新 `docs/TASKS.md`

**Review & Merge**：
- `/review-pr <number>`：typecheck + test + build + 代码审查
- 需要时浏览器视觉验证
- 合格则 merge + 更新 TASKS.md + changelog

**小任务（1-2 文件）**：nodex 可直接在 main 上修复，或使用 Claude Code subagent 完成，无需走 PR 流程。

## 实现策略

- 涉及 **3+ 文件**的特性，必须先出计划（文件清单 + 修改思路）获得确认，再分步实现，每步后 `npm run typecheck`
- nodex（主 Agent）可通过 Chrome 扩展做视觉验证；Dev Agent 自行 `typecheck` + `vitest` + `build` 自验
...
三个环境各有独立 extension key，可同时加载到 Chrome 中共存。图标右下角的彩色圆点区分当前版本。源文件 `docs/icon-drafts/Vector.svg`（简化猫剪影），工具栏图标 `public/icon*/`（16/24/32px 撑满画布）+ 商店图标（48/128px 带边距）。`action.default_icon` 单独设置工具栏尺寸。
