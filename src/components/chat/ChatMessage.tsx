...
const ACTION_BUTTON = 'inline-flex h-7 w-7 items-center justify-center rounded-full text-foreground-tertiary transition-colors hover:bg-foreground/4 hover:text-foreground disabled:cursor-not-allowed disabled:text-foreground-tertiary/40 disabled:hover:bg-transparent';
...
export function ChatMessage({
  ...
  function renderBranchNavigator(): ReactNode {
    ...
    const canGoPrev = branches.currentIndex > 0;
    const canGoNext = branches.currentIndex < branches.ids.length - 1;
  ...
  return (
    <div className={`group flex w-full ${isUser ? 'justify-end' : 'justify-start'} ${grouped ? 'mt-1' : 'mt-4 first:mt-0'}`}>
      <div className={`flex max-w-[88%] flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {isUser ? (
          isEditing ? (
            <div className="w-full min-w-[220px] rounded-lg border border-border bg-background px-3 py-2 shadow-sm">
              <textarea
                ref={textareaRef}
                rows={1}
                value={editText}
                onChange={(event) => setEditText(event.target.value)}
                className="min-h-10 w-full resize-none bg-transparent text-sm leading-6 text-foreground outline-none placeholder:text-foreground-tertiary"
              />
        ...
        {showToolbar && (
          <div className="mt-1 flex items-center gap-1 rounded-full border border-border/80 bg-background/85 px-1 py-0.5 shadow-sm backdrop-blur-sm opacity-0 transition-opacity group-hover:opacity-100">
            {isUser ? (
              <button
                type="button"
                onClick={() => {
