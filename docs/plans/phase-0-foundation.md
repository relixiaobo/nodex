...
- API key 不应存在客户端（安全，Phase 1 迁移到 D1 加密存储）
...
来源：现有路由挂载模式（`server/src/index.ts`）
...
**Phase 0 的 API key 传递问题**：`streamProxy` 的 body 只包含 `{ model, context, options }`，没有 `apiKey` 字段。需要在 `streamFn` 中修改 context 或用 custom header 传递 API key。

**解决方案**：在 context 中注入 apiKey（server 端从 context 中提取并删除后再传给 pi-ai）：
...
streamFn: async (model, context, options) => {
  ...
  return streamProxy(model, {
    ...context,
    _apiKey: apiKey,  // server 端提取
  }, { ...options, authToken: token, proxyUrl: SYNC_API_URL });
}
```

或者更干净的方案：**不使用 streamProxy，直接 fetch**（保持与 streamProxy 相同的 SSE 解析逻辑，但可以自定义 body 格式）。这需要从 pi-agent-core 导入 `ProxyMessageEventStream` 和 `processProxyEvent`——需要确认它们是否被导出。

**推荐方案**：在服务端，从 request body 中读取 apiKey，不需要修改 streamProxy。Body 是 `JSON.stringify({ model, context, options })`，server 端可以从 `context._apiKey` 或单独的 body 字段读取。实测后确定最佳路径。
...
### 风险 2: streamProxy API key 传递（中）

`streamProxy` 的 body 格式是固定的 `{ model, context, options }`，没有 `apiKey` 字段。需要找到一种方式将 API key 传递给 server。

**解决方案优先级**：
1. 在 `context` 中注入 `_apiKey` 字段，server 端提取后删除再传给 pi-ai
2. 不使用 `streamProxy`，自己写 fetch（保持 SSE 协议兼容）
3. 用自定义 header（但 CORS 可能需要调整）
