...
| 2 | Clip Page | — | ✅ 可用 | 就地将当前节点转为网页剪藏（改名+标签+字段），不切换页面（`/clip`） |
| 3 | Search node | — | 🟡 禁用 | 占位，灰色不可点击（待 Search Node UI） |
| 4 | Field | `>` | ✅ 可用 | 等价 `>` 触发（新增字段） |
| 5 | Reference | `@` | ✅ 可用 | 等价 `@` 触发（引用节点） |
| 6 | Image / file | — | 🟡 禁用 | 占位，灰色不可点击 |
| 7 | Heading | `!` | ✅ 可用 | 文本格式 mark（与 Bold/Italic 同级），toggle heading mark |
| 8 | Checkbox | `⌘↩` | ✅ 可用 | 等价 `Cmd+Enter`（为节点启用/切换 checkbox 状态） |
| 9 | Checklist | — | 🟡 禁用 | 占位，灰色不可点击 |
| 10 | Start live transcription | — | 🟡 禁用 | 占位，灰色不可点击 |
| 11 | More commands | `⌘K` | ✅ 可用 | 打开 Cmd+K CommandPalette |
...
| 2026-02-15 | 新增 `Clip Page` 命令（`/clip`） | 网页剪藏入口从 Sidebar 迁移至 slash command，就地转换当前节点 |
| 2026-02-16 | 点亮 `Heading` 命令（`/heading`） | 复用文本格式化能力，与 Floating Toolbar 的 Heading mark 保持一致 |
