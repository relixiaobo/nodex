...
| **卡片层** | NodePanel（面包屑 + 内容区） | `bg-background` (`#F5F4EE`)，`rounded-xl`，`shadow-card`（极轻两层阴影） |
...
极轻两层阴影，仅定义卡片边缘——介于"无阴影"（桌面层）和"五层纸叠"（浮层）之间。
...
| 3 | **桌面与纸张** | 阴影仅在卡片边缘（`shadow-card`）和浮层（`shadow-paper`）？卡片内部无阴影？ |
...
| 我要加阴影了？ | 卡片内部禁止。卡片边缘用 `shadow-card`，浮层用 `shadow-paper`（§6） |
