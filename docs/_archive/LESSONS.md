...
## Cloudflare Workers 部署经验（2026-02-23）

### Wrangler 双环境配置

- 使用 `[env.staging]` / `[env.production]` 隔离资源，共享代码
- **顶层 D1/R2/vars 配置保留给 `wrangler dev`（本地开发）**，env 段独立配置各自的 D1/R2
- `workers.dev` 子域名是**账号级别**的（一个账号一个），staging/production 通过 Worker name 自动区分：`nodex-sync-staging.xxx.workers.dev` vs `nodex-sync.xxx.workers.dev`
- 子域名需要先在 Dashboard 注册（首次部署时 wrangler 会提示 onboarding URL）

### 网络问题（中国大陆）

- **Cloudflare API 需要代理**：`api.cloudflare.com` 直连超时，需要 `HTTPS_PROXY`
- **wrangler 需要大写 + 小写环境变量都设置**：`HTTPS_PROXY` + `https_proxy` + `HTTP_PROXY` + `http_proxy`（只设小写不稳定）
- `ping api.cloudflare.com` 通不代表 HTTPS 通（ICMP vs TCP/TLS 是不同协议）
- **部署命令模板**：`export HTTPS_PROXY=http://127.0.0.1:7890 HTTP_PROXY=http://127.0.0.1:7890 https_proxy=http://127.0.0.1:7890 http_proxy=http://127.0.0.1:7890 && wrangler deploy --env staging`

### R2 binding

- Sync 的 push/pull 路径强依赖 R2（`SYNC_BUCKET`），**不能注释掉 R2 binding**
- 即使暂时不用 sync，binding 也必须保留（Wrangler 部署时会校验 binding 对应的资源是否存在）
- R2 需要在 Dashboard 开通后才能 CLI 创建 bucket
- 本地 `wrangler dev` 使用文件模拟 R2，不需要真实 bucket

### wrangler secret put 注意事项

- 通过管道传入值：`echo "value" | wrangler secret put NAME --env staging`
- 并行设置多个 secrets 可能因网络不稳定失败，建议**逐个串行**执行
- `--env` 参数必须指定，否则 secret 会设到 default env（本地 dev 不需要 secrets）

### Better Auth HTTPS cookie 前缀

- Better Auth 在 `baseURL` 为 HTTPS 时，cookie 名自动加 `__Secure-` 前缀
- 本地 dev（HTTP）：`better-auth.session_token`
- Staging/Production（HTTPS）：`__Secure-better-auth.session_token`
- **读 cookie 时必须兼容两种名称**（先查 `__Secure-` 版本，fallback 到无前缀版本）

### 进程资源耗尽

- 在 Claude Code 中并行启动过多后台 bash 任务（特别是 wrangler/node 进程）会导致 `fork failed: resource temporarily unavailable`
- **恢复方法**：`pkill -f wrangler` 清理僵尸进程
- **预防**：避免同时启动超过 2-3 个 wrangler 后台任务
