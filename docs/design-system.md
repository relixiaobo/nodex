...
文档流内**一律不使用阴影**。浮层使用 Paper Shadow（五层纸叠阴影）替代边框，营造物理纸张堆叠感。
...
#### Paper Shadow（`.shadow-paper`）

```css
box-shadow:
  0 0 0 1px rgba(0,0,0, 0.015),       /* 物理边缘线 */
  0 -1px 2px rgba(255,255,255, 0.6),   /* 顶部高光 */
  0 2px 5px -1px rgba(0,0,0, 0.05),    /* 接触面投影 */
  0 6px 10px -3px rgba(0,0,0, 0.03),   /* 中间过渡 */
  0 12px 20px -4px rgba(0,0,0, 0.04);  /* 远端扩散 */
```

定义在 `src/assets/main.css` 中作为全局工具类。所有浮层统一使用 `bg-background shadow-paper rounded-lg`。
...
| 3 | **零 Z 深度** | 文档流内是否完全无阴影？浮层是否用 `.shadow-paper`？ |
...
| 我要加阴影了？ | 文档流内禁止。浮层用 `.shadow-paper`（五层纸叠），不用 border |
