...
文档流内**一律不使用阴影**。浮层通过**多层阴影**营造"一张暖纸轻放在另一张暖纸上"的物理感——不靠色差区分层级，而靠光影暗示厚度。

| 层级 | 场景 | 表现 |
|------|------|------|
| **文档层** | 大纲、面板、内容区 | 无阴影、无边框，纯 Paper 底色 |
| **浮层** | Dropdown、Popover、Tooltip | Paper 底色 + 五层纸叠阴影（见下方） |
| **遮罩层** | Modal、Dialog | Paper 底色 + 五层纸叠阴影 + `backdrop: rgba(0,0,0,0.3)` |
...
Tailwind 工具类：
```
shadow-[0_0_0_1px_rgba(0,0,0,0.015),0_-1px_2px_rgba(255,255,255,0.6),0_2px_5px_-1px_rgba(0,0,0,0.05),0_6px_10px_-3px_rgba(0,0,0,0.03),0_12px_20px_-4px_rgba(0,0,0,0.04)]
```
...
Nodex 的 icon 不是"三张纸摆在一起"，是**从同一个根长出来的三片叶子**。
...
- 容器：`rounded-lg bg-background p-1` + 五层纸叠阴影（见 §6 Paper Shadow）
- 项：`px-2 py-1 text-base`
- 选中项：`bg-primary text-primary-foreground`
- 高亮项（hover / 键盘导航）：`bg-foreground/4`
...
| 3 | **纸上叠纸** | 文档流内是否完全无阴影？浮层是否使用五层纸叠阴影（非 border）？ |
...
| 我要加阴影了？ | 文档流内禁止。浮层用五层纸叠阴影（§6 Paper Shadow），不用 border |
