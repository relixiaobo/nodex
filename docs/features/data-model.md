...
为同时满足“一切皆节点”和查询执行效率，重构后采用三层分工：

1. **写模型（Write Model）= 节点图**  
   Source of Truth。所有业务真值只写入 Node + Tuple + Metanode + AssociatedData。
2. **读模型（Read Model）= 查询事实（Query Facts）**  
   由写模型派生的索引/投影层，仅用于执行加速，不承载业务真值。
3. **语言模型（Language Model）= NQL AST**  
   标准化查询表达（`docs/features/nql.md`），统一 Query Builder 与自然语言入口。

关键约束：

- 不把业务真值直接写入 Query Facts；
- 不把 SQL 当用户层查询语言；
- 所有 Query Facts 都可由节点图重建。
...
NodexNode
  ...
  └── sourceUrl?: string            ← 来源 URL（已废弃，待清理）
```
...
| `sourceUrl` | `source_url` | `TEXT`（已废弃，待迁移删除） |
...
### 查询事实层（Read Model，派生层）

> 本节是目标架构约定。Query Facts 用于加速 NQL 执行，不是业务真值源。

建议至少维护三类投影（命名可调整）：

| 投影 | 作用 |
|------|------|
| `node_tag_edges` | `node_id -> tag_def_id` 关系，支持 has_tag / includeSubtags |
| `node_field_facts` | 字段值标准化事实（文本/数字/布尔/引用/日期） |
| `node_ref_edges` | 节点引用与内联引用关系，支持 backlinks / links-to |

日期字段推荐在 `node_field_facts` 里保留双形态：

- `date_node_id`（业务真值引用，指向 `journalPart` day 节点）
- `date_epoch`（派生比较值，用于 before/after/between）

这些事实表可以全量重建，不要求手工维护或人工编辑。
...
| `sourceUrl` | 来源 URL（已废弃，待清理） | V1 后改为 `#web_clip` 字段承载 |
...
| Group | `Tuple [NDX_A09, fieldDefId]`（Group key 预留常量） | `SYS_A.GROUP_FIELD`（实现时补定义） |
...
搜索表达式的写模型形态是 Tuple 树，查询语言形态统一为 NQL AST。推荐流程：

`Tuple 树 <-> NQL（Canonical）-> 执行计划`

当前可兼容旧形式 `Tuple [SYS_A15, tagDefId, filterTupleId]`，重构目标是 `SYS_A15 -> nqlRootTupleId`。

**不要**把搜索条件序列化为 DSL 字符串（如 `"#task AND status:TODO"`）。
...
| 可视化 | Query Builder 直接渲染 Tuple/NQL | 需要 DSL → AST → UI 往返转换 |
| 组合 | AND/OR/NOT = 嵌套 Tuple 父子关系 | 需要解析优先级和括号 |

实现 Query Builder 时，每个条件对应 Tuple 节点，并同步映射到 NQL predicate。
...
| 2026-02-18 | 增加“写模型/读模型/NQL 语言模型”三层架构 | 同时保持节点真值与查询执行效率，统一 NL/Builder 查询入口 |
