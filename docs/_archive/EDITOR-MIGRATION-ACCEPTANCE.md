...
| **Cmd+Shift+ArrowUp** | 当前节点上移 | [ ]（已修复：保留原字符偏移；待手工回归） |
| **Cmd+Shift+ArrowDown** | 当前节点下移 | [ ]（已修复：保留原字符偏移；待手工回归） |
...
- [ ] 格式变更可撤销（已修复：编辑态不再被 nav undo 抢占；待手工回归）
- [x] 撤销后光标位置正确
...
| 触发器 | 验收步骤 | 通过 |
|--------|---------|------|
| **# HashTag** | 输入 `#ta` → 标签选择器出现，显示匹配标签 | [x]（已通过：无穿模，可鼠标点击） |
...
| **@ 选择（空节点）** | 空节点输入 `@ref` 选择 → 节点转为引用节点 | [x]（已通过：无穿模，可鼠标点击） |
...
- [x] 编辑器失焦 → 工具栏消失
- [x] 选区跨越 inline ref atom → 工具栏仍然出现（测试方法：先插入一个 inline ref，然后从它前一个字拖拽到它后一个字，确认工具栏会出现）
