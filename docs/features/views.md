...
视图配置在语言层统一映射到 NQL（`docs/features/nql.md`），在存储层仍由 Tuple 持久化。
...
| Sort By | ❌ |
| 视图切换 UI | ❌ |
| ViewDef ↔ NQL 映射层 | ⚠️ 规范已定义，未落地执行器 |
...
| `NDX_A.GROUP_FIELD` | `NDX_A09`（预留） | 分组字段 |
...
```
ViewDef Metanode:
  └── Tuple [NDX_A09, fieldDefId]   ← 分组字段
```

> 约定：`NDX_A09` 作为 Group key 预留常量。实现时需在 `system-nodes.ts` 中定义 `SYS_A.GROUP_FIELD`（值为 `NDX_A09`）。
...
兼容期可读取旧结构 `[SYS_A15, tagDefId]`，但新写入统一走 `nqlRootTupleId`。

这意味着搜索节点 + Table 视图 = 类似 Notion Database 的体验。详见 `docs/features/search.md`。

## ViewDef 与 NQL 的关系

ViewDef 负责持久化配置，NQL 负责标准化执行表达：

| ViewDef Tuple | NQL 字段 | 说明 |
|---------------|----------|------|
| `[SYS_A18, filterTupleId]` | `where` | 过滤表达式树 |
| `[SYS_A19, dir] + [SYS_A20, fieldId]` | `sort[]` | 排序规则（可多级） |
| `[NDX_A09, fieldId]` | `group[]` | 分组字段 |
| `_view` + `[SYS_A16, viewDefId]` | `view` | 展示模式与视图实例 |

执行路径：

1. 从 ViewDef 读取 Tuple 配置；
2. 规范化成 NQL；
3. NQL 编译执行（Query Facts + 节点图回补）；
4. 将结果交给具体视图渲染。
...
3. 搜索节点的 Filter 与普通节点的 Filter 共享同一数据模型（Tuple），并统一映射到同一套 NQL 字段
4. ViewDef 可通过 supertag 模板继承 — tagDef 模板中的 `[SYS_A16, viewDefId]` Tuple 在标签应用时实例化，实例 ViewDef 的 Filter/Sort/Group Tuple 继承模板默认配置
5. Query Facts 是执行层派生索引，不是配置真值；配置真值永远在 ViewDef Tuple
...
| 2026-02-16 | Group By 使用 `NDX_A09` Tuple 存储 | 与 Filter(SYS_A18)/Sort(SYS_A19+20) 对称；Tana 无导出对应常量 |
| 2026-02-16 | 视图配置可通过 supertag 模板继承 | tagDef 模板中定义默认视图 → 应用标签时实例化 |
| 2026-02-18 | ViewDef 配置统一映射到 NQL | 统一 Query Builder 与自然语言查询入口，减少双协议分叉 |
