...
- Bug 修复：更新 `docs/issues.md`（Open/Closed 状态、标题、关闭日期）
...
本项目可能有多个 Agent 并行开发。遵守以下规则避免冲突：

### Git 分支规范

- **禁止在 `main` 分支直接开发**。所有改动通过功能分支 → PR → merge 合入 main
- 分支命名：`codex/<feature-name>`（如 `codex/webclip-capture`）
- 每个 Agent 使用独立的 clone 目录，不共享工作区
- 开始新任务前：`git checkout main && git pull && git checkout -b codex/<name>`
...
# 2. 开发、提交（遵循常规 commit 规范）
git add <files> && git commit -m "feat: <summary>"
...
- **高风险共享文件**（多 Agent 极易同时修改）：
  - `src/stores/node-store.ts` — 几乎所有功能都要加 action
  - `src/components/outliner/OutlinerItem.tsx` — 几乎所有 UI 功能都要改
  - `docs/issues.md` — 跟踪文件
  - `docs/TESTING.md` — test-sync 要求每次都更新
- 分工时按**文件归属**划分，不仅按功能划分
- 谁后合并谁处理冲突
...
- 每个 Agent 认领的功能在 issue 中标注负责人
- 不确定是否会与其他 Agent 冲突时，先在 issue 中声明要改哪些文件
...
- 每次修复 bug，补一个对应 Vitest 回归用例（与 `docs/issues.md` 条目对应）
