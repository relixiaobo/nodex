...
- **存放位置**：创建后自动放入 `SEARCHES` 工作区容器（`{workspaceId}_SEARCHES`，已在 `system-node-registry.ts` 注册）
...
| 方面 | Tana | Nodex |
|------|------|-------|
| 查询配置存储 | Metanode → Tuple 树（SYS_A15） | children 子节点树（queryCondition 节点） |
...
| 条件值存储 | Tuple 属性 | 子节点（与 fieldEntry 统一） |
...
Nodex 的选择：按使用频率分层，最常见的操作最简单，复杂查询交给 AI。三层共享同一个 queryCondition 数据模型，只是创建入口不同。Query Builder 式的条件树编辑不作为 Phase 1-2 的目标。
...
| **Undo/Redo** | reference 创建/删除进入 Loro undo 栈 |
...
| `SYS_V.HAS_TAG` (`SYS_V19`)、`SYS_V.CHILD_OF` (`SYS_V53`) 等操作符常量 | `src/types/system-nodes.ts` | 已定义（Tana 兼容，Nodex 使用可读字符串替代） |
| `SEARCHES` 工作区容器 | `src/types/system-nodes.ts` → WORKSPACE_CONTAINERS | 已注册 |
...
| 旧版 `search-service.ts`（Supabase 时代） | `src/services/search-service.ts` | 遗留代码，使用旧 Metanode/Tuple 架构，不应使用 |
...
- `src/lib/loro-doc.ts` — queryCondition 节点的 Loro 读写
...
3. **大量结果的性能**：Tana 单个 search node 最多 580 个结果。如果结果过多（>100），是否分页或限制物化 reference 数量？`refreshSearchResults` 应批量操作、单次 `commitDoc()`，避免逐 reference 提交。
4. **结果节点被删除时**：reference 的 targetId 指向的节点被 trash 时，reference 自动移除还是标记为失效？
5. **重复 search node 防护**：L0 点击标签时，如果已存在同一 tagDefId 的 search node，是导航到现有的还是创建新的？（倾向导航到现有，避免 SEARCHES 容器膨胀）
...
**交付标准**：`typecheck` → `vitest` → `build` 全绿。种子数据中可看到 search node + 结果。
...
> 以下为 Tana 支持的全部搜索条件，供实现时逐项对照。Nodex 的 QueryOp 命名可能与 Tana 关键字不同，但语义应覆盖。
