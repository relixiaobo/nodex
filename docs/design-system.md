...
|--------|---------|----------|------|
| Paper | `--background` | `#F5F4EE` | NodePanel 卡片底色（暖纸色） |
| Desk | `--background-recessed` | `#EEEEE8` | 桌面层底色（Chat、GlobalTools 所在层） |
...
v5.0 标签为**排印化**（纯文本着色，无背景）。11 色均为低饱和纸质色调，在 `#F5F4EE` 底色上 ≥ 3:1 对比度。通过 hash 分配（gray 除外，仅手动选择）。

| 序号 | 色相分类 | 名称 | 色值 | 色相描述 |
|------|----------|------|------|----------|
| 0 | Red | Vintage Red | `#A6535B` | 复古红，经典泛红 |
| 1 | Orange | Terracotta | `#BA6C43` | 陶土橙，明亮温暖 |
| 2 | Amber | Antique Gold | `#9B7C38` | 古董金，金属质感 |
...
| 10 | Yellow | Harvest Yellow | `#8B8422` | 收获黄，#highlight 专用 |
...
| `text-xl` | 20px | 28px | 500 | 页面标题（Journal / Settings 等） |
...
| ==高亮== | `rgba(247,236,139,0.6)` Soft Banana + `mix-blend-mode: multiply`（荧光笔墨水感） |
...
| **节点间距** | **4px** | 兄弟节点垂直间距（gap-1，OutlinerView + OutlinerItem 子容器） |
| 缩进步长 | 28px | 每层缩进距离 |
...
整体布局采用**两层 Z 轴模型**：桌面（Desk）是低层背景，纸张卡片（Card）浮在桌面上。

| 层级 | 场景 | 表现 |
|------|------|------|
| **桌面层** | Chat、GlobalTools、resize handle | `bg-background-recessed` (`#EEEEE8`)，无阴影 |
| **卡片层** | NodePanel（面包屑 + 内容区） | `bg-background` (`#F5F4EE`)，`rounded-xl`，`shadow-panel-right`（宽屏时向右投影） |
...
卡片层内部（大纲、面板内容）**不使用阴影**。阴影仅出现在卡片边缘（卡片与桌面的分界）和浮层。

#### 卡片右侧投影（`.shadow-panel-right`）

```css
box-shadow:
  2px 0 4px -1px rgba(0, 0, 0, 0.06),
  6px 0 16px -4px rgba(0, 0, 0, 0.06);
```

仅在宽屏 + Chat 打开时生效，NodePanel 卡片向右侧的 Chat 区域投影，强化 Z 轴分离感。

#### Chrome-tab 异形连接（`.tab-connector-right`）

窄屏时面包屑呈 Chrome 标签页形状，标签右下角通过 `::after` 伪元素和 `box-shadow` 实现凹形内角，连接标签与桌面。
...
> Logo 升级进行中。设计方向：蜷缩的猫剪影，Sage Green 单色，几何曲线美感。确定后更新此节。

#### 设计约束

| 约束 | 说明 |
|------|------|
| **单色** | Sage Green `#5E8E65` 为唯一颜色，透明背景 |
| **醒目** | 颜色占比 > 留白，在 Chrome 深色 toolbar 上清晰可辨 |
| **几何美** | 曲线精确、数学感，参考 Apple logo 的简洁优雅 |
| **可爱** | 温暖有生命力，不冰冷 |
...
WXT 自动从 `public/icon/` 发现并写入 `manifest.json`。
...
| 3 | **桌面与纸张** | 阴影仅在卡片边缘（`shadow-panel-right`）和浮层（`shadow-paper`）？卡片内部无阴影？ |
...
| 问 | 答 |
|----|----|
| 我要加颜色了？ | 只能用三功能色（Green/Amber/Red）、Ink 三级、Tag 11 色、Info/Success 之一 |
| 我要加阴影了？ | 卡片内部禁止。卡片边缘用 `shadow-panel-right`，浮层用 `shadow-paper`（§6） |
| 我要加圆角了？ | 大纲节点 = 0，容器 = 8/12px，按钮 = pill |
| 我要加背景了？ | 卡片层用 Paper (`--background`)；桌面层用 Desk (`--background-recessed`)；hover 用 `foreground/4`；选中用 `primary-muted` |
| 这个字号对吗？ | 只能从 11/13/15/17/20/24 中选，基准正文 15px |
| 这个字重对吗？ | 只用 400/500；用户手动加粗例外 (600) |
| 这个元素要一直显示吗？ | 非核心交互元素应使用呼吸式显现（hover 时出现） |
| 标签怎么渲染？ | 纯文本着色，无背景 badge，色值从 Tag 11 色 hash 分配 |
