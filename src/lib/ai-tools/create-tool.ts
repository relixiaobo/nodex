...
const createToolParameters = Type.Object({
  ...
  text: Type.Optional(Type.String({ description: 'Structured text content. First line = root node. Supports #tags, field:: value, [[Name^id]] references, [X]/[ ] checkboxes. Children use 2-space indentation. See tool description for full format.' })),
...
export const createTool: AgentTool<typeof createToolParameters, unknown> = {
  ...
  description: [
    ...
    '',
    'Pass structured text. Everything is indented under the root node:',
    '',
    '```',
    'Root node name #tag1 #tag2',
    ...
    '',
    'Structure rules:',
    '- Line 1 = root node. May include inline #tags, [[Name^id]] references, or leading [X]/[ ].',
    '- If line 1 is exactly [[Name^id]], a reference node is created.',
    '- Lines after line 1 at indent 0: only root-level metadata (#tags, [X]/[ ], or field:: lines).',
    '- All children and fields of a node use 2-space indent relative to their parent.',
    '- Max nesting depth = 3 levels.',
    '',
    'Field rules:',
    '- field:: value → single value (inline).',
    '- field:: (no inline value) + indented lines → multi-value. Each indented line = one value.',
    '- IMPORTANT: After a multi-value field:: line, all subsequent lines at the next indent level become field values until a line at the same or lower indent appears.',
    '- Field values can be references: [[Name^id]].',
    '',
    'Tag and reference rules:',
    '- #tagName anywhere in a line → apply tag (auto-creates if tag does not exist).',
    '- [[Display^nodeId]] → inline reference (resolved by nodeId, display text is for readability).',
    '- A line that is exactly [[Display^nodeId]] → reference node (the whole node is a link).',
    '',
    'Other:',
    '- [X] = checked, [ ] = unchecked.',
    '- Lines may start with "- " (bullet prefix is stripped).',
    '- data parameter: set non-content properties (description, color, codeLanguage, showCheckbox).',
    '',
    'Examples:',
    '- Simple: text: "Buy groceries #task"',
    '- With fields: text: "Meeting #meeting\\n  Status:: Done\\n  Location:: Room A"',
    '- Reference: text: "[[Report^abc123]]"',
    '- Tree: text: "Project #project\\n  Phase 1\\n    Design\\n    Research\\n  Phase 2\\n    Development"',
    '- Multi-value: text: "Task #task\\n  Assignees::\\n    Alice\\n    Bob\\n  Notes\\n    First note"',
    '',
    '## Search nodes',
    '',
    'Pass type: "search" with name and rules.',
    '- rules.query = text filter on node name/description',
    '- rules.searchTags = tag display names (AND logic; unresolved tags are skipped)',
    '- rules.fields = field value filter by display name, e.g. {"Status": "Done"}',
    '- rules.linkedTo = backlink target node ID',
    '- rules.scopeId = restrict to subtree of this node',
    '- rules.after / rules.before = creation date bounds (YYYY-MM-DD)',
    '- rules.sortBy = created|modified|name|refCount[:asc|desc] (relevance is NOT supported — it is runtime-only)',
    '',
    'Example: node_create(type: "search", name: "Open tasks", rules: {searchTags: ["task"], fields: {"Status": "Todo"}})',
    '',
    '## Duplicate mode',
    '',
    'Pass duplicateId to deep-copy an existing node.',
    '',
